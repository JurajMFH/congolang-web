...
// --- Lesson/Flashcard Data Handling ---
let currentLessonData = null;
let currentPhraseIndex = 0;
let currentTranslationIndex = 0;

async function loadLessons() {
    const lessonId = "core_lessons"; // Using a consistent ID for the main lesson file
    const flashcardSection = document.getElementById('flashcard-container-section');
    const topicTitle = document.getElementById('lesson-topic');
    ...
    try {
        ...
        if (typeof IdbManager !== 'undefined') {
            await IdbManager.saveLesson(lessonId, currentLessonData);
            console.log("[IDB] Lesson cached successfully");
        }
        
        renderLessonUI(currentLessonData);
    } catch (error) {
        console.warn("Offline or network error! Attempting to load from IndexedDB...");
        
        // 2. Offline fallback from IndexedDB
        if (typeof IdbManager !== 'undefined') {
            const cachedLesson = await IdbManager.getLesson(lessonId);
            if (cachedLesson) {
                currentLessonData = cachedLesson;
                renderLessonUI(currentLessonData);
            } else {
                topicTitle.innerText = "Ste offline a túto lekciu ešte nemáte stiahnutú.";
                console.error('No cached data available');
            }
        } else {
            console.error('IdbManager not found');
            topicTitle.innerText = "Erreur de chargement des données.";
        }
    }
}
...
function renderLessonUI(data) {
    ...
    if (data && data.phrases.length > 0) {
        ...
        // Add control listeners (ensure they are only added once)
        ...
        newNextBtn.addEventListener('click', showNextCard);
        newPrevBtn.addEventListener('click', showPrevCard);
    }
}

function renderCard() {
    const display = document.getElementById('flashcard-display');
    const phrase = currentLessonData.phrases[currentPhraseIndex];
    const translation = phrase.translations[currentTranslationIndex];

    display.innerHTML = `
        <p class="card-french">${phrase.french}</p>
        <p class="card-translation">${translation.text}</p>
        <span class="card-lang-tag">${getLangName(translation.lang)}</span>
    `;

    // Play audio if available (mock logic for now as files don't exist)
    if (translation.audio) {
        console.log(`[Audio] Playing: ${translation.audio}`);
...
function showNextCard() {
    ...
    if (currentTranslationIndex >= currentLessonData.phrases[currentPhraseIndex].translations.length) {
        currentTranslationIndex = 0;
        currentPhraseIndex = (currentPhraseIndex + 1) % currentLessonData.phrases.length;
    }
    renderCard();
}
...
function showPrevCard() {
    ...
    if (currentTranslationIndex < 0) {
        currentPhraseIndex = (currentPhraseIndex - 1 + currentLessonData.phrases.length) % currentLessonData.phrases.length;
        currentTranslationIndex = currentLessonData.phrases[currentPhraseIndex].translations.length - 1;
    }
    renderCard();
}
